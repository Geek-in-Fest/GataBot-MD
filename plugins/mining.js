let handler = async (m, { conn }) => {
  ...
  if (user.pickaxe > 0) {
    if (new Date() - user.lastmining > 3000000) {
      ...
      conn.sendBut(
        m.chat,
        `Tunggu 🕓${timers} lagi, untuk menambang`,
        `${botol}`,
        `⋮☰ Back`,
        `.menu`,
        m
    ...
    conn.sendBut(
      m.chat,
      `Kamu Tidak Mempunyai *⛏️Pickaxe* untuk menambang\nBuat Lah Pickaxe menggunakan string kayu dan batu!`,
      `${wm}`,
      `Craft Pickaxe`,
      `.craft pickaxe`,
      m
