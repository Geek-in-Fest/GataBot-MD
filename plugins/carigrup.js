...
let handler = async (m, { text, usedPrefix, command }) => {
  ...
  conn.sendButtonLoc(
    m.chat,
    await (await fetch(image)).buffer(),
    teks,
    wm,
    "Jangan Di Pencet",
    "huuu",
...
async function carigroup(search, searchby = "name") {
  let { data } = await axios.get(
    global.API("http://ngarang.com", "/link-grup-wa/daftar-link-grup-wa.php", {
  ...
  let results = [];
  $("#content > div.entry.clearfix > div.wa-chat").each(function (
    index,
    element
  ) {
    let subject = $(this)
      ...
      .trim();
    let link = $(this)
      .find("div > div.wa-chat-message > a")
      .attr("href")
      .trim();
    results.push({
      subject,
