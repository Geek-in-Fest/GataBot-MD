...
import { generateForwardMessageContent } from "@adiwajshing/baileys";
import { jidDecode } from "@adiwajshing/baileys";
import { getBinaryNodeChild } from "@adiwajshing/baileys";
...
export default {
  ...
  END:VCARD
    ...
    conn.getBusinessProfile = async (jid) => {
      ...
      const profiles = getBinaryNodeChild(
        getBinaryNodeChild(results, "business_profile"),
        "profile"
      );
  ...
  smsg: (conn, m, hasParent) => {
    ...
    if (m.key) {
      ...
      m.chat = conn.decodeJid(
        m.key.remoteJid || (m.msg && m.msg.groupId) || ""
      );
    ...
    if (m.message) {
      ...
      if (m.quoted) {
        ...
        m.getQuotedObj = m.getQuotedMessage = () => {
          ...
          let q = ((conn.chats[m.quoted.chat] || {}).messages || {})[
            m.quoted.id
          ];
        ...
        if (m.quoted.url || m.quoted.directPath)
          m.quoted.download = () =>
            conn.downloadM(
              m.quoted,
              m.quoted.mtype.toLowerCase().replace(/message/i, "")
            );
    ...
    m.reply = async (text, chatId, options) => {
      ...
      let { data } = await conn.getFile(
        await (await require("node-fetch")(pp)).buffer()
      );
      conn.reply(chatId ? chatId : m.chat, text, m, {
        contextInfo: {
          ...
          externalAdReply: {
            ...
            body: wm,
            sourceUrl: "https://youtube.com/channel/UCBtUyjfIclyuu7yXKS0dAMw",
            thumbnail: data,
          },
        },
        options,
    ...
    if (m.msg && m.msg.url)
      m.download = () =>
        conn.downloadM(m.msg, m.mtype.toLowerCase().replace(/message/i, ""));
