...
let handler = async (
  ...
  if (!args || !args[0])
    throw `contoh:\n${
      usedPrefix + command
    } https://www.youtube.com/watch?v=yxDdj_G9uRY`;
  ...
  m.reply(
    isLimit
      ? `Ukuran File: ${filesizeF}\nUkuran file diatas ${limit} MB, download sendiri: ${dl_link}`
      : global.wait
  );
  if (!isLimit)
    conn.sendFile(
      ...
      null,
      {
        ...
        contextInfo: {
          externalAdReply: {
            ...
            mediaType: 2,
            thumbnail: await (
              await fetch("https://telegra.ph/file/76f08bd0aeb06c3e0c0d5.jpg")
            ).buffer(),
...
handler.help = ["mp3", "a"].map(
  (v) => "yt" + v + ` <url> [server: ${servers.join(", ")}]`
);
