...
let handler = async (m, { usedPrefix, command, conn, args }) => {
  ...
  let result = res.result.reverse().filter(({ mime }) => /video/i.test(mime)),
    video,
    index;
  ...
  conn.sendFile(
    m.chat,
    video,
    "twitter" + /video/.test(ress.mime) ? ".mp4" : ".png",
    `
*Name:* ${res.name}
*Mime:* ${ress.mime}
`.trim(),
...
async function twitter(url) {
  if (!/http(?:s)?:\/\/(?:www\.)?twitter\.com\/([a-zA-Z0-9_]+)/i)
    throw "Link invalid!";
  ...
  let res = await fetch(
    "https://www.expertsphp.com/instagram-reels-downloader.php",
    {
      ...
      headers: {
        ...
        cookie:
          "_ga=GA1.2.783835709.1637038175; _gid=GA1.2.880188214.1637038175; __gads=ID=5b4991618655cd86-22e2c7aeadce00ae:T=1637038176:RT=1637038176:S=ALNI_MaCe3McPrVVswzBEqcQlgnVZXtZ1g",
