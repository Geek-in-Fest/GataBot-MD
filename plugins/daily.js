...
let handler = async (m, { conn, usedPrefix, isPrems }) => {
  if (db.data.users[m.sender].level < 1)
    conn.sendBut(
      m.chat,
      `Naikkan level kamu dengar mengetikkan\n${usedPrefix}levelup`,
      wm,
      "Naikkan Level",
      ".levelup",
      m
  ...
  db.data.users[m.sender].exp += isPrems
    ? prem * db.data.users[m.sender].level
    : free * db.data.users[m.sender].level;
  conn.sendBut(
    m.chat,
    `+${
      isPrems
