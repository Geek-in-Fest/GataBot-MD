...
let handler = async (m, { conn, command, args, usedPrefix, DevMode }) => {
  ...
  global.db.data.users[m.sender].pancing =
    global.db.data.users[m.sender].pancing || 0;
  let botol = global.wm;
...
`;
  ...
  try {
    if (/craft|Crafting/i.test(command)) {
      ...
      switch (type) {
        case "pickaxe":
          if (user.kayu < 10 || user.iron < 5 || user.string < 20)
            return m.reply(
        ...
        case "sword":
          ...
          global.db.data.users[m.sender].kayu -= global.db.data.users[
            m.sender
          ].iron -= 10;
          global.db.data.users[m.sender].string -= 20;
