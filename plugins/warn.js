let handler = async (m, { conn, args }) => {
  ...
  if (user.warn * 1 < 3) {
    ...
    m.reply(
      "Kamu di warn oleh OWNER Atau MODERATOR!!, dan sekarang kamu punya *" +
        (user.warn + 1) +
        "* WARN. Ingat Jika kamu mendapat warn 4 kali kamu akan otomatis ke banned",
      mention
    );
  } else if (user.warn * 1 > 2) {
    let reason =
      (args.length > 0 || args[1] ? args.slice(1).join(" ") : "4 kali WARN") ||
      "4 kali WARN";
    ...
    m.reply(
      "*Kamu Terbanned karena telah mendapatkan 4 kali warn*\n *HUBUNGI* \n" +
        global.owner
